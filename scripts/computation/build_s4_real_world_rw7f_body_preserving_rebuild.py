#!/usr/bin/env python
"""Build RW7f TREE_007 body-preserving hinge rebuild for S4.

RW7f consumes the RW7 work order but intentionally bypasses the RW7b boolean-rebuilt body pieces after RW8f proved that RW7b truncates P2/P3. It starts from the exact RW2 source body solids and adds the refined TREE_007 hardware as an external body-preserving rebuild.

For the TREE_007 variant RW7f creates:
...
Scope boundary: RW7f is a CAD/mesh rebuild and body-envelope gate. It does not export STL/3MF, does not run a slicer, does not print a coupon, and does not validate physical hingeability.
"""
...
RW8F_PATH = RESULT_ROOT / "rw8f_tree007_geometry_regression_audit_report.json"
JSON_PATH = RESULT_ROOT / "rw7f_body_preserving_rebuild_report.json"
DOC_PATH = ROOT / "docs" / "S4_RW7F_BODY_PRESERVING_REBUILD.md"
OUT_DIR = RESULT_ROOT / "cad_hardware_body_preserving_rebuild"
MANIFEST_PATH = OUT_DIR / "rw7f_body_preserving_rebuild_manifest.json"
...
REFINED_PARAMETERS_MM = {
    "pin_radius_mm": 1.25,
    "pin_diameter_mm": 2.50,
    "clearance_mm": 0.30,
    "radial_envelope_mm": 3.40,
    "boss_width_mm": 2.40,
    "web_penetration_mm": 1.80,
    "web_tube_overlap_mm": 0.05,
    "pin_cap_thickness_mm": 0.80,
    "pin_cap_gap_mm": 0.25,
}
...
OPEN_BLOCKERS = [
    "RW7f hardware geometry is body-preserving and still unprinted/unmeasured",
...
def retained_pin_stack(start: np.ndarray, end: np.ndarray, pin_radius: float, collar_radius: float, collar_thickness: float, collar_gap: float, sections: int = SECTIONS) -> trimesh.Trimesh:
    """One-piece stepped reference pin with larger retention collars.

    The small-radius shaft spans through the active axis and through the small
    collar gaps.  The larger collars are connected to that shaft in the same
    watertight mesh.  This is still a retention placeholder: a real prototype
    may replace one collar with a detachable cap, clip, or off-the-shelf
    fastener stack.
    """
    u = unit(end - start)
    _, v, w = external_basis(start, end)
    length = float(np.linalg.norm(end - start))
    stations = [
        (-collar_gap - collar_thickness, collar_radius),
        (-collar_gap, collar_radius),
        (-collar_gap, pin_radius),
        (length + collar_gap, pin_radius),
        (length + collar_gap, collar_radius),
        (length + collar_gap + collar_thickness, collar_radius),
    ]
    vertices: list[np.ndarray] = []
    for dist, radius in stations:
        center = start + u * dist
    ...
    vertices.append(start + u * stations[0][0])
    end_center = len(vertices)
    vertices.append(start + u * stations[-1][0])
    first = 0
    last = (len(stations) - 1) * sections
    for i in range(sections):
        j = (i + 1) % sections
        faces.append([start_center, first + j, first + i])
        faces.append([end_center, last + i, last + j])
    mesh = trimesh.Trimesh(vertices=np.asarray(vertices, dtype=float), faces=np.asarray(faces, dtype=int), process=True)
    repair.fix_normals(mesh)
    return mesh
...
def export_obj(path: Path, groups: list[tuple[str, trimesh.Trimesh]]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    lines = [
        "# RW7f body-preserving hinge rebuild OBJ",
        "# body-preserving CAD/mesh draft; not STL/3MF, not slicer-ready, not prototype-validated",
    ]
    offset = 0
...
def build_tree_variant(tree_id: str, hardware_rows: list[dict[str, Any]], rebuilt_bodies: dict[str, trimesh.Trimesh], scale: float) -> dict[str, Any]:
    ...
    web_penetration = REFINED_PARAMETERS_MM["web_penetration_mm"] / scale
    web_tube_overlap = REFINED_PARAMETERS_MM["web_tube_overlap_mm"] / scale
    offset_distance = max(outer_radius + clearance, 3.8 / scale)
    ...
    for row in hardware_rows:
        ...
        pin_mesh = retained_pin_stack(
            offset_start,
            offset_end,
            pin_radius,
            outer_radius,
            REFINED_PARAMETERS_MM["pin_cap_thickness_mm"] / scale,
            REFINED_PARAMETERS_MM["pin_cap_gap_mm"] / scale,
        )
        ...
        hardware_component_records.append({
            ...
            "component": pin_name,
            "component_kind": "removable_pin_with_retention_collars_reference",
            "owner_piece": None,
            "axis_offset_model_units": offset_distance,
            "axis_offset_mm": offset_distance * scale,
            "pin_radius_mm": row["pin_radius_mm"],
            "pin_cap_radius_mm": outer_radius * scale,
            "pin_cap_thickness_mm": REFINED_PARAMETERS_MM["pin_cap_thickness_mm"],
            "pin_cap_gap_mm": REFINED_PARAMETERS_MM["pin_cap_gap_mm"],
            "retention_placeholder_present": True,
    ...
    return {
        "tree_id": tree_id,
        "status": "body_preserving_variant_passes_mesh_clearance_connectivity_checks" if variant_passes else "body_preserving_variant_blocked_by_mesh_clearance_or_connectivity_checks",
        "design_parameters": {
...
def body_preservation_records(variant: dict[str, Any], source_bodies: dict[str, trimesh.Trimesh], scale: float) -> list[dict[str, Any]]:
    source_metrics = {piece_id: mesh_metrics(mesh) for piece_id, mesh in source_bodies.items()}
    printed_by_piece = {row["piece_id"]: row["mesh_metrics"] for row in variant["piece_results"]}
    records = []
    for piece_id in PIECE_IDS:
        src = source_metrics[piece_id]
        printed = printed_by_piece[piece_id]
        src_min = np.asarray(src["bounds_min_model"], dtype=float)
        src_max = np.asarray(src["bounds_max_model"], dtype=float)
        pr_min = np.asarray(printed["bounds_min_model"], dtype=float)
        pr_max = np.asarray(printed["bounds_max_model"], dtype=float)
        volume_ratio = float(printed["volume_model_units"] / src["volume_model_units"])
        bbox_contains = bool(np.all(pr_min <= src_min + 1e-6) and np.all(pr_max >= src_max - 1e-6))
        records.append({
            "piece_id": piece_id,
            "source_volume_model_units": src["volume_model_units"],
            "printed_volume_model_units": printed["volume_model_units"],
            "printed_to_source_volume_ratio": volume_ratio,
            "source_volume_mm3": src["volume_model_units"] * scale ** 3,
            "printed_volume_mm3": printed["volume_model_units"] * scale ** 3,
            "source_bounds_min_mm": [float(x * scale) for x in src_min],
            "source_bounds_max_mm": [float(x * scale) for x in src_max],
            "printed_bounds_min_mm": [float(x * scale) for x in pr_min],
            "printed_bounds_max_mm": [float(x * scale) for x in pr_max],
            "passes_volume_lower_bound": volume_ratio >= 0.999,
            "passes_bbox_contains_source_body": bbox_contains,
        })
    return records
...
def build_doc(payload: dict[str, Any]) -> str:
    ...
    piece_rows = []
    for row in payload["body_preservation_records"]:
        piece_rows.append([
            row["piece_id"],
            f"{row['source_volume_mm3']:.1f}",
            f"{row['printed_volume_mm3']:.1f}",
            f"{row['printed_to_source_volume_ratio'] * 100:.2f}%",
            row["passes_volume_lower_bound"],
            row["passes_bbox_contains_source_body"],
            [round(x, 3) for x in row["printed_bounds_max_mm"]],
        ])
    acceptance_rows = [[key, value] for key, value in payload["acceptance"].items()]
    blocker_rows = [[item] for item in payload["open_blockers"]]
    return f"""# S4 RW7f body-preserving TREE_007 rebuild

Status: `{payload['status']}`.

RW7f is the repair route after RW8f confirmed that RW7b truncated P2/P3.  This
stage starts from the exact RW2 source body solids, not from RW7b, and reapplies
the refined TREE_007 hinge hardware as an external body-preserving rebuild.

## Outputs

| artifact | path |
| --- | --- |
| RW7f JSON report | `{rel(JSON_PATH)}` |
| RW7f manifest | `{rel(MANIFEST_PATH)}` |
| TREE_007 assembly OBJ | `{variant['artifacts']['combined_assembly_obj']}` |

## Body preservation gate

All volumes are reported at the 60 mm model scale.

{table(['piece', 'source mm3', 'printed mm3', 'printed/source', 'volume ok', 'bbox contains source', 'printed bbox max mm'], piece_rows)}

## Mesh and clearance summary

| field | value |
| --- | --- |
| printed pieces valid volumes | {variant['summary']['all_printed_pieces_watertight_valid_volumes']} |
| printed pieces single components | {variant['summary']['all_printed_pieces_single_connected_components']} |
| hardware unions valid volumes | {variant['summary']['all_hardware_unions_valid_volumes']} |
| piece-pair positive volume overlaps clear | {variant['summary']['all_piece_pair_positive_volume_overlaps_clear']} |
| pin/body positive volume overlaps clear | {variant['summary']['all_pin_body_positive_volume_overlaps_clear']} |
| pin/pin positive volume overlaps clear | {variant['summary']['all_pin_pin_positive_volume_overlaps_clear']} |
| variant passes mesh/clearance checks | {variant['summary']['variant_passes_mesh_clearance_checks']} |
...
RW7f may unblock a new export/audit cycle only if the body-preservation gate and
mesh/clearance checks pass.  It still does not claim physical hingeability,
fabrication readiness, G-code readiness, static coupon validation, or moving
...
def main() -> None:
    ...
    rw8f = load_json(RW8F_PATH)
    selected_candidate_id = rw7["summary"]["selected_candidate_id"] + "_rw7f_body_preserving_tree007"
    if not module_available("manifold3d"):
        raise SystemExit("manifold3d is required for RW7f body-preserving rebuild")
    scale = float(rw7["selected_candidate"]["parameters"]["scale_mm_per_model_unit"])

    source_bodies = {
        piece_id: load_obj_min(RESULT_ROOT / "meshes" / "body_solids" / f"{piece_id}.obj")
        for piece_id in PIECE_IDS
    }
    ...
    refined_rows = []
    for row in rows_by_tree.get(TARGET_TREE_ID, []):
        refined = dict(row)
        refined["pin_radius_mm"] = REFINED_PARAMETERS_MM["pin_radius_mm"]
        refined["pin_diameter_mm"] = REFINED_PARAMETERS_MM["pin_diameter_mm"]
        refined["clearance_mm"] = REFINED_PARAMETERS_MM["clearance_mm"]
        refined["radial_envelope_mm"] = REFINED_PARAMETERS_MM["radial_envelope_mm"]
        refined["boss_width_mm"] = REFINED_PARAMETERS_MM["boss_width_mm"]
        refined["geometry_status"] = "rw7f_body_preserving_cad_solid_with_retention_placeholder"
        refined["required_cad_features"] = list(refined.get("required_cad_features", [])) + [
            "body-preserving source-solid gate",
            "explicit pin-retention placeholder collars",
        ]
        refined_rows.append(refined)

    variant_results = [build_tree_variant(TARGET_TREE_ID, refined_rows, source_bodies, scale)]
    all_variants_pass = all(v["summary"]["variant_passes_mesh_clearance_checks"] for v in variant_results)
    body_records = body_preservation_records(variant_results[0], source_bodies, scale)
    all_body_volume_preserved = all(row["passes_volume_lower_bound"] for row in body_records)
    all_body_bbox_preserved = all(row["passes_bbox_contains_source_body"] for row in body_records)
    body_preservation_passes = all_body_volume_preserved and all_body_bbox_preserved
    first_params = variant_results[0]["design_parameters"]
    all_gates_pass = all_variants_pass and body_preservation_passes

    payload: dict[str, Any] = {
        "report_id": "S4-RW7F-BODY-PRESERVING-REBUILD-2026-06-23",
        "date": DATE,
        "case_id": CASE_ID,
        "status": "rw7f_body_preserving_rebuild_passes_body_mesh_clearance_gates" if all_gates_pass else "rw7f_body_preserving_rebuild_blocked_by_body_mesh_or_clearance_gate",
        "selected_candidate_id": selected_candidate_id,
        "precondition": {
            "rw7_report": rel(RW7_PATH),
            "rw8f_report": rel(RW8F_PATH),
            "rw8f_status": rw8f.get("status"),
        },
        "backend_capability": {
        ...
        "summary": {
            ...
            "body_preservation_passes": body_preservation_passes,
            "all_body_volume_lower_bounds_preserved": all_body_volume_preserved,
            "all_body_bboxes_contain_source_bodies": all_body_bbox_preserved,
            ...
            "next_export_cycle_unblocked": all_gates_pass,
        },
        "body_preservation_records": body_records,
        ...
        "open_blockers": [
            "RW7f repairs the body-envelope regression, but it is still unprinted and unmeasured",
            "material strength, anisotropy, friction, wear, and pin retention are untested",
            "pin insertion/removal and assembly sequence are not physically tested",
            "STL/3MF export and slicer/layer preview must be rerun from RW7f outputs",
            "static fit coupon and moving prototype have not been printed or measured",
        ],
        "acceptance": {
            "rw7_report_present": RW7_PATH.exists(),
            "rw8f_report_present": RW8F_PATH.exists(),
            "rw8f_blocks_prior_rw9": rw8f.get("acceptance", {}).get("rw9_static_coupon_allowed") is False,
            "source_bodies_used_instead_of_rw7b": True,
            "manifold3d_backend_available": module_available("manifold3d"),
            "single_tree007_rebuild_created": len(variant_results) == 1 and variant_results[0]["tree_id"] == TARGET_TREE_ID,
        ...
        "next_task": "RW8g export/audit RW7f outputs if and only if next_export_cycle_unblocked is true; otherwise adjust RW7f hardware attachment/clearance.",
    }
    ...
    print(json.dumps({
        "status": payload["status"],
        "selected_candidate_id": selected_candidate_id,
        "body_preservation_passes": payload["summary"]["body_preservation_passes"],
        "all_variants_pass_mesh_clearance_checks": payload["summary"]["all_variants_pass_mesh_clearance_checks"],
        "next_export_cycle_unblocked": payload["summary"]["next_export_cycle_unblocked"],
