"""TREE_007 P2-P3 edge-edge targeted endgame guard.

This audit follows the P2-P3 edge-branch projection-component probe. It targets
only the 37 child subsegments that remain uncovered after the 4463/4500 child
coverage on the selected edge-branch workflow.

The endgame guard uses the locally best named SAT axis at each subcell midpoint
and applies the signed projection-component support/extrema bound on that local
axis. It is adaptive:

1. Try the local best-axis component guard on the original 37 children.
2. Refine all 37 to max coordinate delta 0.01953125 degrees.
3. Refine only remaining hard children to max coordinate delta 0.001220703125
   degrees.

This is finite numeric evidence for the selected P2-P3 edge-branch workflow. It
...
REPORT_NAME = "tree007_p2p3_edge_branch_targeted_endgame_guard_report.json"
...
import audit_historical_s4_tree007_p2p3_edge_branch_projection_component_bound_probe as projection_probe  # noqa: E402
...
def best_named_axis_with_unit(poly_a: list[np.ndarray], poly_b: list[np.ndarray], labels_a: list[str], labels_b: list[str]) -> dict:
    best = None
    for name, axis in branch_probe.classify.axis_records(poly_a, poly_b, labels_a, labels_b):
...
def local_best_axis_component_guard(
    ...
    left_degrees = projection_probe.vector_to_degrees(tree, left)
    right_degrees = projection_probe.vector_to_degrees(tree, right)
    center_degrees = projection_probe.vector_to_degrees(tree, center)
    ...
    def vertices(piece_id: str, labels: list[str]) -> list[np.ndarray]:
        return support_probe.vertices_for_labels(transformed, indices, piece_id, labels)

    lower_support_positive = projection_probe.component_displacement_bound_for_labels(
        case, transforms, delta_by_hinge, paths_by_piece, vertices(state["lower_piece"], lower_support), state["lower_piece"], unit, "positive"
    )
    upper_support_negative = projection_probe.component_displacement_bound_for_labels(
        case, transforms, delta_by_hinge, paths_by_piece, vertices(state["upper_piece"], upper_support), state["upper_piece"], unit, "negative"
    )
    lower_support_negative = projection_probe.component_displacement_bound_for_labels(
        case, transforms, delta_by_hinge, paths_by_piece, vertices(state["lower_piece"], lower_support), state["lower_piece"], unit, "negative"
    )
    lower_non_support_positive = projection_probe.component_displacement_bound_for_labels(
    ...
    upper_support_positive = projection_probe.component_displacement_bound_for_labels(
        case, transforms, delta_by_hinge, paths_by_piece, vertices(state["upper_piece"], upper_support), state["upper_piece"], unit, "positive"
    )
    upper_non_support_negative = projection_probe.component_displacement_bound_for_labels(
...
def projected_remaining_children(
    case: dict,
    tree: dict,
    indices: dict,
    ...
    for base in residuals:
        ...
        for child_index, (left, right) in enumerate(sensitivity.subdivide_vector_segment(base["left_vector"], base["right_vector"], SOURCE_REFINEMENT_DELTA_DEGREES)):
            ...
            component = projection_probe.component_support_guard_from_segment(case, tree, indices, labels_by_piece, paths_by_piece, left, right, branch)
            projected_reason = projection_probe.projected_child_reason(evaluation, component)
            ...
            children.append(
                {
                    ...
                    "source_edge": projection_probe.source_edge_descriptor(base["source_node_ids"]),
                    "assigned_branch_name": branch,
                    "previous_projected_reason": projected_reason,
                    "previous_best_axis_name": evaluation["context"]["best_axis_name"],
                    "left_vector": left,
                    "right_vector": right,
                }
            )
...
def build_report() -> dict:
    ...
    prior_projected_certified = selection_context["projected_reason_counts"].get("covered:whole_piece", 0) + selection_context["projected_reason_counts"].get("covered:stable_support", 0) + selection_context["projected_reason_counts"].get("covered:projection_component", 0)
    prior_child_count = sum(selection_context["projected_reason_counts"].values())

    return {
        "case_id": CASE_ID,
        "status": "tree007_p2p3_edge_branch_targeted_endgame_guard_completed",
        "source_reports": [
        ...
        "local_best_axis_rule": "At each subcell midpoint choose the named SAT axis with minimum overlap for P2-P3, then apply the signed projection-component support/extrema guard on that local axis.",
        "thresholds": {
        ...
        "summary_metrics": {
            ...
            "prior_projected_certified_child_count": prior_projected_certified,
            "coarse_child_count_after_endgame": prior_child_count if adaptive_completed else None,
        },
        ...
        "limitations": [
            "This is a finite adaptive guard for the 37 children left by the previous projection-component probe, not a symbolic theorem.",
            "It completes only the selected TREE_007 P2-P3 edge-edge branch workflow after replacing hard child cells by finer subcells.",
            "It does not cover the P2-P3 face-normal parent branches left_face:B-M_AB-M_CD and right_face:B-M_AB-M_CD.",
            "It does not certify theta=0, the full continuous 3-parameter component, physical hinge thickness, offsets, mesh export, or printability.",
        ],
    }
