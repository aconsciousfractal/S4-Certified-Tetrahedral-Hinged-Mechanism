"""TREE_021 P0-P2 theta-edge projection-component bound probe.

The residual-base classifier showed that the remaining P0-P2 bottleneck after
0.15625-degree refinement is entirely made of theta-source edges. This probe
replaces the previous norm displacement bound on support vertices with a
component bound along the fixed branch separator axis.

For a vertex offset p from a hinge axis with unit direction u and separator unit
n, the projection displacement under a small hinge perturbation epsilon is

    n dot (R_epsilon p - p) = A sin(epsilon) + B (1 - cos(epsilon))

where A = n dot (u cross p) and B = (n dot u)(u dot p) - n dot p.

For |epsilon| <= h, this gives conservative signed bounds for movement in the
positive and negative separator directions. The probe keeps the existing
support/extrema structure and the existing displacement safety factor.

This is still a finite numeric probe, not a complete symbolic certificate.
"""
...
REPORT_NAME = "p0p2_theta_projection_component_bound_probe_report.json"
...
def component_support_guard_from_segment(
    ...
    # Gap can close only if the lower maximum moves positive or the upper minimum moves negative.
    ...
    # The lower support is a maximum; it can lose support only by moving negative while a non-support vertex moves positive.
    ...
    # The upper support is a minimum; it can lose support only by moving positive while a non-support vertex moves negative.
...
def projected_child_reason(evaluation: dict, component: dict) -> str:
    ...
    if evaluation["axis_relation"] != "same_assigned_edge_branch":
        return f"uncovered:axis_switch:{evaluation['axis_relation']}"
    if component.get("signed_component_certified"):
        return "covered:theta_projection_component"
    if not component.get("signed_extrema_stable"):
        return "uncovered:theta_projection_component_stability_failed"
    return "uncovered:theta_projection_component_margin_failed"
...
def compact_child_record(base: dict, child_index: int, evaluation: dict, component: dict, old_reason: str, projected_reason: str) -> dict:
    return {
        ...
        "center_angle_degrees_by_hinge": evaluation["center_angle_degrees_by_hinge"],
        "axis_relation": evaluation["axis_relation"],
        "best_axis_name": evaluation["best_axis_name"],
        "branch_gap": evaluation["branch_gap"],
        "old_support_margin": evaluation["support"].get("support_margin"),
        "old_minimum_stability_margin": evaluation["support"].get("minimum_stability_margin"),
...
def build_report() -> dict:
    ...
    for base in residuals:
        ...
        source = base_classification.source_edge_descriptor(base["source_node_ids"])
        for child_index, (left, right) in enumerate(sensitivity.subdivide_vector_segment(base["left_vector"], base["right_vector"], TARGET_REFINEMENT_DELTA_DEGREES)):
            ...
            if (not old_covered) and projected_reason == "covered:theta_projection_component":
                ...
                add_example(examples["newly_certified_by_theta_projection_component"], compact_child_record(base, child_index, evaluation, component, old_reason, projected_reason))
            if (not old_covered) and evaluation["axis_relation"] == "same_assigned_edge_branch" and component.get("absolute_component_certified"):
            ...
            if not projected_covered:
                ...
                add_example(examples["remaining_uncovered_after_theta_projection_component"], compact_child_record(base, child_index, evaluation, component, old_reason, projected_reason))
    ...
    return {
        "case_id": CASE_ID,
        "status": "p0p2_theta_projection_component_bound_probe_completed",
        "source_reports": [
        ...
        "limitations": [
            ...
            "The probe covers only the residual same-branch TREE_021 P0-P2 base subsegments selected by the previous refinement-sensitivity workflow.",
            "Axis-switch children are not certified by the same-branch theta projection-component rule.",
            "The P0-P3/P1-P2 shared-edge residual contacts, TREE_007 mirror transfer, theta=0 limit, full 3-parameter component, and physical hinge thickness are outside this report.",
        ],
    }
