#!/usr/bin/env python
"""Build RW7e TREE_007 hinge mechanical refinement for S4.

RW7e consumes the RW7 work order and RW7b boolean-rebuilt body pieces,
but targets only TREE_007 with a stricter hinge design after the RW8c
mechanical sanity audit rejected the RW7d baseline as not final-prototype-ready.
...
JSON_PATH = RESULT_ROOT / "rw7e_hinge_mechanical_refinement_report.json"
DOC_PATH = ROOT / "docs" / "S4_RW7E_HINGE_MECHANICAL_REFINEMENT.md"
OUT_DIR = RESULT_ROOT / "cad_hardware_mechanical_refinement"
MANIFEST_PATH = OUT_DIR / "rw7e_hinge_mechanical_refinement_manifest.json"
...
OPEN_BLOCKERS = [
    "RW7e hardware geometry is a compact refinement, but still unprinted and unmeasured",
...
def export_obj(path: Path, groups: list[tuple[str, trimesh.Trimesh]]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    lines = [
        "# RW7e hinge mechanical refinement OBJ",
...
def build_doc(payload: dict[str, Any]) -> str:
    ...
    return f"""# S4 RW7e hinge mechanical refinement

Status: {payload['status']}.

RW7e materializes a compact mechanical refinement of the TREE_007 hinge hardware for the selected candidate `{payload['selected_candidate_id']}` on top of the RW7b boolean-rebuilt body pieces.  It creates separate variants for `TREE_007` and `TREE_021`; these are alternative hinge layouts, not a single six-hinge assembly.
...
RW7e does not claim physical hingeability, direct printability, STL/3MF readiness, G-code readiness, fabrication readiness, static coupon validation, or moving prototype validation.

## Next task

RW8d may export the passing RW7e TREE_007 refinement to STL/3MF and a viewer bundle.  A physical claim still remains blocked until RW9/RW10 measurements.
"""
...
def main() -> None:
    ...
    selected_candidate_id = rw7["summary"]["selected_candidate_id"] + "_rw7e_tree007_compact_refinement"
    ...
    for row in rows_by_tree.get(TARGET_TREE_ID, []):
        ...
        refined["geometry_status"] = "rw7e_refined_cad_solid_with_retention_placeholder"
        refined["required_cad_features"] = list(refined.get("required_cad_features", [])) + ["explicit pin-retention placeholder collars"]
        refined_rows.append(refined)
    rows_by_tree = {TARGET_TREE_ID: refined_rows}
    ...
    payload: dict[str, Any] = {
        "report_id": "S4-RW7E-HINGE-MECHANICAL-REFINEMENT-2026-06-23",
        "date": DATE,
        "case_id": CASE_ID,
        "status": "rw7e_tree007_compact_refinement_passes_mesh_clearance_connectivity_checks" if all_variants_pass else "rw7e_tree007_compact_refinement_blocked_by_mesh_clearance_or_connectivity_checks",
        ...
        "open_blockers": ["RW7e increased connector-web penetration and tightened hinge parameters to repair body-boss connectivity; mechanical strength remains untested"] + OPEN_BLOCKERS,
        "acceptance": {
            ...
            "single_tree007_refinement_created": len(variant_results) == 1 and variant_results[0]["tree_id"] == TARGET_TREE_ID,
        ...
        "next_task": "RW8d should export the passing RW7e TREE_007 refinement and create a final viewer/inspection bundle before RW9 coupon work.",
    }
