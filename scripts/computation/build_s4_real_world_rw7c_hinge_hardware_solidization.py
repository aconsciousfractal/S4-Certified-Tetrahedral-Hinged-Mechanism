#!/usr/bin/env python
"""Build RW7c hinge hardware solidization variants for S4.

RW7c consumes the RW7 hardware work order and RW7b boolean-rebuilt body pieces.
...
For each tree variant RW7c creates:
...
Scope boundary: RW7c is a CAD/mesh solidization and clearance gate. It does
...
JSON_PATH = RESULT_ROOT / "rw7c_hinge_hardware_solidization_report.json"
DOC_PATH = ROOT / "docs" / "S4_RW7C_HINGE_HARDWARE_SOLIDIZATION.md"
OUT_DIR = RESULT_ROOT / "cad_hardware_solidization"
MANIFEST_PATH = OUT_DIR / "rw7c_hinge_hardware_solidization_manifest.json"
...
OPEN_BLOCKERS = [
    "RW7c hardware geometry is a baseline printable hinge layout, not an optimized mechanical design",
...
def export_obj(path: Path, groups: list[tuple[str, trimesh.Trimesh]]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    lines = [
        "# RW7c hinge hardware solidization OBJ",
...
def build_tree_variant(tree_id: str, hardware_rows: list[dict[str, Any]], rebuilt_bodies: dict[str, trimesh.Trimesh], scale: float) -> dict[str, Any]:
    ...
    web_penetration = 0.005  # model units = 0.30 mm at the current scale.
    ...
    variant_passes = all([all_printed_pieces_valid, all_unions_valid, all_piece_pair_clear, all_pin_body_clear, all_pin_pin_clear])
    return {
        "tree_id": tree_id,
        "status": "hardware_solidization_variant_passes_mesh_clearance_checks" if variant_passes else "hardware_solidization_variant_blocked_by_mesh_clearance_checks",
        "design_parameters": {
...
def build_doc(payload: dict[str, Any]) -> str:
    ...
    return f"""# S4 RW7c hinge hardware solidization

Status: {payload['status']}.

RW7c materializes baseline hinge hardware solids for the selected candidate `{payload['selected_candidate_id']}` on top of the RW7b boolean-rebuilt body pieces.  It creates separate variants for `TREE_007` and `TREE_021`; these are alternative hinge layouts, not a single six-hinge assembly.

## Outputs

| artifact | path |
| --- | --- |
| RW7c JSON report | `{rel(JSON_PATH)}` |
| RW7c manifest | `{rel(MANIFEST_PATH)}` |
...
RW7c does not claim physical hingeability, direct printability, STL/3MF readiness, G-code readiness, fabrication readiness, static coupon validation, or moving prototype validation.

## Next task

RW8 may export the passing RW7c variant assemblies to STL/3MF and run slicer/layer preview.  A physical claim still remains blocked until RW9/RW10 measurements.
"""
...
def main() -> None:
    ...
    if not module_available("manifold3d"):
        raise SystemExit("manifold3d is required for RW7c hardware solidization")
    ...
    payload: dict[str, Any] = {
        ...
        "date": DATE,
        "case_id": CASE_ID,
        "status": "rw7c_hardware_solidization_variants_pass_mesh_checks_export_gate_unblocked" if all_variants_pass else "rw7c_hardware_solidization_variants_blocked_by_mesh_checks",
        ...
        "next_task": "RW8 export passing RW7c variant assemblies to STL/3MF and run slicer/layer preview; physical claims remain blocked until coupon/prototype measurements.",
    }
