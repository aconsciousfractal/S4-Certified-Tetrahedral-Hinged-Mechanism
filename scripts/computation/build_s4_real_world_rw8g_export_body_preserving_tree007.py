#!/usr/bin/env python
"""RW8g STL/3MF export and slicer preflight for the body-preserving TREE_007 variant.

RW8g consumes the passing RW7f body-preserving rebuild variant and produces
...
JSON_PATH = RESULT_ROOT / "rw8g_export_body_preserving_tree007_report.json"
OUT_DIR = RESULT_ROOT / "export_slicer_preflight_rw7f_tree007"
MANIFEST_PATH = OUT_DIR / "rw8g_export_body_preserving_tree007_manifest.json"
DOC_PATH = ROOT / "docs" / "S4_RW8G_EXPORT_BODY_PRESERVING_TREE007.md"
...
RW8g exports the passing body-preserving TREE_007 hardware variant to millimeter-scaled STL/3MF artifacts and runs layer-preview checks.  The export uses the local STL writer, stdlib 3MF writer, printability gate, and pure-Python layer preview.  External G-code slicing is attempted only if a supported PrusaSlicer/CuraEngine CLI is detected.

## Outputs

| artifact | path |
| --- | --- |
| RW8g JSON report | `{rel(JSON_PATH)}` |
...
RW8g does not claim physical hingeability or fabrication readiness.  Passing component STL/3MF export and layer preview only means the body-preserving TREE_007 component meshes are exportable and inspectable in a print-oriented format; assembly-preview exports are diagnostic and are not fabrication files.  Coupon and moving-prototype evidence are still required.
...
RW9 should use the body-preserving TREE_007 component set for a static fit coupon plan and measured pin/boss/hole tolerance record before any moving prototype claim.
"""
...
def main() -> None:
    clean_export_dir()
    source_path = RW7F_PATH
    source_stage = "RW7f"
    rw7c = load_json(source_path)
    if not rw7c.get("summary", {}).get("next_export_cycle_unblocked"):
        raise RuntimeError("RW7f does not unblock export cycle; refusing RW8g export")
    ...
    for variant in passing_source_variants:
        ...
        variant_payload = {
            "tree_id": tree_id,
            "status": "rw8g_variant_exports_and_layer_preview_pass" if variant_summary["all_component_exports_pass"] else "rw8g_variant_export_preflight_blocked",
    ...
    status = (
        "rw8g_body_preserving_tree007_component_exports_layer_preview_and_external_gcode_pass_coupon_gate_unblocked"
        if component_exports_pass and external_gcode_generated
        else "rw8g_body_preserving_tree007_component_exports_and_papp_layer_preview_pass_external_slicer_cli_blocked"
        if component_exports_pass and not external_slicer_available
        else "rw8g_body_preserving_tree007_component_export_preflight_blocked"
    )
    ...
    payload: dict[str, Any] = {
        "report_id": "S4-RW8G-EXPORT-BODY-PRESERVING-TREE007-2026-06-23",
        "date": DATE,
        "case_id": CASE_ID,
        "status": status,
        ...
        "summary": {
            ...
            "rw9_static_coupon_unblocked": bool(component_exports_pass and rw7c.get("summary", {}).get("body_preservation_passes")),
            "body_preserving_tree007_export": True,
        },
        ...
        "next_task": "RW9 static fit coupon plan/print/measurement using the RW8g body-preserving TREE_007 component set; physical hingeability remains blocked until RW10 moving prototype measurements.",
    }
