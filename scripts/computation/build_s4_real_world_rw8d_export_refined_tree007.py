#!/usr/bin/env python
"""RW8d STL/3MF export and slicer preflight for the refined TREE_007 variant.

RW8d consumes the passing RW7e mechanical-refinement variant and produces
...
RW7E_PATH = RESULT_ROOT / "rw7e_hinge_mechanical_refinement_report.json"
JSON_PATH = RESULT_ROOT / "rw8d_export_refined_tree007_report.json"
OUT_DIR = RESULT_ROOT / "export_slicer_preflight_rw7e_tree007"
MANIFEST_PATH = OUT_DIR / "rw8d_export_refined_tree007_manifest.json"
DOC_PATH = ROOT / "docs" / "S4_RW8D_EXPORT_REFINED_TREE007.md"
...
RW8d exports the passing refined TREE_007 hardware variant to millimeter-scaled STL/3MF artifacts and runs layer-preview checks.  The export uses the local STL writer, stdlib 3MF writer, printability gate, and pure-Python layer preview.  External G-code slicing is attempted only if a supported PrusaSlicer/CuraEngine CLI is detected.

## Outputs

| artifact | path |
| --- | --- |
| RW8d JSON report | `{rel(JSON_PATH)}` |
...
RW8d does not claim physical hingeability or fabrication readiness.  Passing component STL/3MF export and layer preview only means the refined TREE_007 component meshes are exportable and inspectable in a print-oriented format; assembly-preview exports are diagnostic and are not fabrication files.  Coupon and moving-prototype evidence are still required.
...
RW9 should use the refined TREE_007 component set for a static fit coupon plan and measured pin/boss/hole tolerance record before any moving prototype claim.
"""
...
def main() -> None:
    ...
    for variant in passing_source_variants:
        ...
        variant_payload = {
            "tree_id": tree_id,
            "status": "rw8d_variant_exports_and_layer_preview_pass" if variant_summary["all_component_exports_pass"] else "rw8d_variant_export_preflight_blocked",
    ...
    status = (
        "rw8d_refined_tree007_component_exports_layer_preview_and_external_gcode_pass_coupon_gate_unblocked"
        if component_exports_pass and external_gcode_generated
        else "rw8d_refined_tree007_component_exports_and_papp_layer_preview_pass_external_slicer_cli_blocked"
        if component_exports_pass and not external_slicer_available
        else "rw8d_refined_tree007_component_export_preflight_blocked"
    )
    ...
    payload: dict[str, Any] = {
        "report_id": "S4-RW8D-EXPORT-REFINED-TREE007-2026-06-23",
        "date": DATE,
        "case_id": CASE_ID,
        "status": status,
        ...
        "next_task": "RW9 static fit coupon plan/print/measurement using the RW8d refined TREE_007 component set; physical hingeability remains blocked until RW10 moving prototype measurements.",
    }
