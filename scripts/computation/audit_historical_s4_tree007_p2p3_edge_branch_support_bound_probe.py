"""TREE_007 P2-P3 edge-edge branch support-bound probe.

This audit follows the TREE_007 P2-P3 edge-branch lower-bound probe at max
coordinate delta 0.625 degrees. It targets only the lower-bound failed leaf
subsegments and tries a tighter fixed-axis support-extrema guard.

The stable_support guard is finite numeric evidence: the midpoint branch axis is
held fixed, only the projection-extremal support vertices absorb motion along
that axis, and non-support vertices must be unable to overtake the support
extrema under their own local displacement bounds.
"""
...
REPORT_NAME = "tree007_p2p3_edge_branch_support_bound_probe_report.json"
...
import audit_historical_s4_tree007_p2p3_edge_branch_lower_bound_probe as probe  # noqa: E402
...
def build_report() -> dict:
    ...
    parents = probe.parent_branch_segments(case, tree, paths_by_piece, signs_by_tree)

    total_subsegment_count = 0
    lower_bound_certified_count = 0
    lower_bound_failed_count = 0
    ...
    stable_by_assigned_branch = Counter()
    stable_by_axis_relation = Counter()
    ...
    for parent in parents:
        ...
        for sub_index, (left, right) in enumerate(probe.subdivide_segment(parent, TARGET_MAX_COORDINATE_DELTA_DEGREES)):
            ...
            if support.get("stable_support_certified"):
                ...
                stable_by_assigned_branch[branch] += 1
                stable_by_axis_relation[relation] += 1
                ...
                if not support.get("separated_at_center"):
                    remaining_reason_counts["not_separated_at_center"] += 1
    ...
    combined_certified = lower_bound_certified_count + stable_support_count
    return {
        "case_id": CASE_ID,
        "status": "tree007_p2p3_edge_branch_support_bound_probe_completed",
        "source_reports": [
        ...
        "target": {
            ...
            "max_coordinate_delta_degrees": TARGET_MAX_COORDINATE_DELTA_DEGREES,
            "branches": probe.TARGET_BRANCHES,
        },
        ...
        "summary_metrics": {
            "edge_branch_subsegment_count": total_subsegment_count,
            "lower_bound_certified_count": lower_bound_certified_count,
            "lower_bound_failed_count": lower_bound_failed_count,
            ...
            "combined_edge_branch_certified_count": combined_certified,
            "combined_edge_branch_uncovered_count": total_subsegment_count - combined_certified,
            "edge_branch_backlog_closed_by_combined_probe": combined_certified == total_subsegment_count,
        },
        "failed_lower_bound_by_assigned_branch": dict(failed_by_assigned_branch.most_common()),
        "failed_lower_bound_by_axis_relation": dict(failed_by_axis_relation.most_common()),
        "stable_support_by_assigned_branch": dict(stable_by_assigned_branch.most_common()),
        "stable_support_by_axis_relation": dict(stable_by_axis_relation.most_common()),
        ...
        "limitations": [
            ...
            "The stable_support guard is a fixed-axis support-extrema guard; it does not cover arbitrary axis motion.",
            "The report covers only TREE_007 P2-P3 edge-edge branch leaf subsegments at max coordinate delta 0.625 degrees.",
        ...
        json.dumps(
            {
                ...
                "failed_lower_bound_by_axis_relation": report["failed_lower_bound_by_axis_relation"],
